...
class BookController(BaseController[Book, BookCreate, BookUpdate]):
    ...
    async def update_by_user(self, book_in: BookUpdate) -> Book:
        # Save image locally
        cover_url = book_in.cover_url
        meta = await BookHelper.download_image(cover_url)
        if meta:
            book_in.cover_url = meta["url"]
            meta["ref_id"] = str(book_in.id)
            meta["ref_type"] = "book"
            self.fm_controller.save(FileMetaCreate(**meta))

        return super().update_by_user(book_in)
    ...
    async def get_by_isbn(self, isbn: str, check: bool = True) -> BookBase:
        # Check if ISBN exists
        if check:
    ...
    def query_cover(self, book_id: UUID) -> QueryResult[FileMeta]:
        ...
        return self.fm_controller.query(query)
