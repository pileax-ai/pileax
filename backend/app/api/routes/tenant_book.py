...
def save(item_in: TenantBookCreate, session: SessionDep, user_id: CurrentUserId) -> Any:
    return TenantBookController(session, user_id).save(item_in)


@router.api_get("", response_model=TenantBookPublic)
def get(id: uuid.UUID, session: SessionDep, user_id: CurrentUserId) -> Any:
    return TenantBookController(session, user_id).get(id)


@router.api_put("", response_model=TenantBookPublic)
def update(item_in: TenantBookUpdate, session: SessionDep, user_id: CurrentUserId) -> Any:
    return TenantBookController(session, user_id).update(item_in)


@router.api_delete("")
def delete(id: uuid.UUID, session: SessionDep, user_id: CurrentUserId) -> Any:
    return TenantBookController(session, user_id).delete(id)
...
def query(query: PaginationQuery, session: SessionDep, user_id: CurrentUserId) -> Any:
    return TenantBookController(session, user_id).query(query)
...
def get_details(id: uuid.UUID, session: SessionDep, user_id: CurrentUserId) -> Any:
    return TenantBookController(session, user_id).get_details(id)
...
def query_details(query: PaginationQuery, session: SessionDep, user_id: CurrentUserId) -> Any:
    return TenantBookController(session, user_id).query_details(query)
