<template>
  <section class="o-icon-select-view">
    <header class="row justify-between items-center search">
      <q-input v-model="term" placeholder="Search icons" debounce="200" outlined>
        <template #prepend>
          <q-icon name="search" />
        </template>
        <template v-slot:append>
          <q-icon name="close" @click="term = ''" class="cursor-pointer q-mr-xs" v-if="term" />


        </template>
      </q-input>

      <section class="row col-12 justify-between selected-icon">
        <div>
          <q-chip class="bg-accent" size="1.2rem" square v-if="selected">
            <q-avatar color="primary" text-color="white">
              <q-icon :name="selected" />
            </q-avatar>
            {{ selected }}

            <q-icon name="close" size="1rem" class="q-ml-sm cursor-pointer" @click="selected=''" />
          </q-chip>
        </div>
        <q-btn-dropdown color="primary" :label="iconSetTitle" class="bg-primary text-white" flat>
          <q-list>
    ...
    <section class="row icons" :class="isMobile ? 'justify-around' : 'justify-start'">
      <q-btn v-for="(item, index) in iconList" :key="index"
             class="text-readable"
             flat
             @click="onClick(item.name)" v-close-popup>
